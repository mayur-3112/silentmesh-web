...
export default function RuntimeNavigation() {
  ...
  return (
    <header className={`pointer-events-auto fixed left-0 top-0 right-0 z-[60] flex w-full items-center justify-between transition-all duration-500 ${
      scrolled 
        ? 'bg-[#05070b]/85 backdrop-blur-xl border-b border-white/5 py-5 px-8 md:py-6 md:px-16 lg:px-24 shadow-[0_4px_30px_rgba(0,0,0,0.5)]' 
        : 'py-10 px-8 md:py-12 md:px-16 lg:px-24'
    }`}>
      ...
      >
        ...
        <div className="relative flex h-7 w-7 items-center justify-center rounded bg-gradient-to-br from-[#00c9a7] to-cyan-500 p-[2px] shadow-[0_0_18px_rgba(0,201,167,0.35)] transition-transform duration-500 group-hover:rotate-[90deg]">
          <div className="h-full w-full rounded-[2px] bg-[#05070b] flex items-center justify-center">
            <div className="h-2 w-2 rounded-sm bg-[#00c9a7] animate-pulse shadow-[0_0_8px_#00c9a7]" />
          </div>
        </div>

        {/* Text Logo */}
        <span 
          className="font-mono text-xl font-black tracking-[0.3em] text-white uppercase transition-all duration-300 group-hover:tracking-[0.35em] md:text-2xl lg:text-3xl select-none"
          style={{
            textShadow: "0 0 25px rgba(0, 201, 167, 0.35)"
          }}
        >
          SILENTMESH
        </span>
      ...
      {/* Desktop nav */}
      <div className="hidden items-center gap-12 font-mono text-xs md:text-sm tracking-[0.25em] text-white/60 md:flex">
        {navItems.map((item) => (
          <button
            key={item.target}
            data-cursor="hover"
        ...
        <button
          data-cursor="hover"
          className="rounded-full border border-[#00c9a7]/35 bg-[#00c9a7]/5 px-7 py-3 text-xs md:text-sm font-bold tracking-[0.2em] text-[#00c9a7] transition-all duration-300 hover:border-[#00c9a7]/70 hover:bg-[#00c9a7]/15 hover:shadow-[0_0_18px_rgba(0,201,167,0.25)]"
